...
const onboardingCheckoutOptions = () => {
  ...
  const { offerings, currentOffering, availablePackages, isLoading, error, refetchOfferings } = useSubscription();
  ...
  return (
    <View className=" flex flex-col gap-10 h-full w-full px-4 justify-center items-center">
      ...
      <View className="flex flex-row gap-4 w-full ">
        ...
        <TouchableOpacity onPress={() => selectPackage("monthly")} className={`flex flex-1 flex-row gap-4 p-8 rounded-3xl items-center justify-center ${selectedMonthly ? "bg-dark3" : "bg-dark2"}`}>
          ...
          <View className={`p-2 h-8 w-8 items-center justify-center rounded-full ${selectedMonthly ? "bg-accent2 " : "border border-dark3"}`}>
           {selectedMonthly && <SvgXml xml={check} width={16} height={16} color="#E6E6E6"/>}
          </View>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => selectPackage("yearly")} className={`flex flex-1 flex-row gap-4 p-8 rounded-3xl items-center justify-center ${selectedYearly ? "bg-dark3" : "bg-dark2"}`}>
          <View className="flex flex-col gap-1">
            <Text className="text-light2 font-medium text-2xl">Yearly</Text>
            <Text className="text-light3 font-sans text-lg">2,91 $/mo</Text>
          </View>

          <View className={`p-2 h-8 w-8 items-center justify-center rounded-full ${selectedYearly ? "bg-accent2 " : "border border-dark3"}`}>
           {selectedYearly && <SvgXml xml={check} width={16} height={16} color="#E6E6E6"/>}
          </View>
        </TouchableOpacity>
